...
Tef <- read.csv("~/shared-data/Data/Tef/fieldData/teff_dst_team_shared_v1.csv", header = T, sep = ",", fileEncoding="latin1")
nprate <- read.csv("~/shared-data/Data/Tef/fieldData/TefNPrate_July24.csv")
TefNPrate_AdetJuly24 <- read.csv("~/shared-data/Data/Tef/fieldData/TefNPrate_AdetJuly24.csv")
...
```{r}
Tef_joined <- Tef_joined |> dplyr::mutate(NAME_1 = replace(NAME_1, NAME_1 == "Southern Nations, Nationalities", "SNNP"))|> dplyr::mutate(n_rate2 = round_any(n_kgpha, 5), p_rate2 = round_any(p_kgpha, 2), k_rate2 = round_any(k_kgpha, 10)) |> dplyr::mutate(treatment_id = 
paste(n_rate2, p_rate2, k_rate2, sep = "_")) |> dplyr::mutate(location = 
paste(NAME_2, NAME_3, sep = "_"))
dim(Tef_joined)
```
## 5. remove points with < 300 kg/ha, removes 192 data points

```{r}
unique(Tef_joined[Tef_joined$grain_yield_kgpha < 500, ]$treatment_id)

dim(Tef_joined %>% dplyr::filter(grain_yield_kgpha < 500, ))
Tef_joined <- Tef_joined %>% dplyr::filter(grain_yield_kgpha > 500, )
...
Tef_filtered <- Tef_filtered |> dplyr::filter(source != "SG 2000" & grain_yield_kgpha <= 5000)
dim(yld_gr_7k)
dim(Tef_filtered)
```
...
fita <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + (1|trial_id), data= Tef_filtered)

fitb <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + (1|trial_id) + source, data = Tef_filtered)
anova(fita, fitb)

fitc <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + NAME_2 + (1|trial_id) + source, data = Tef_filtered)
anova(fitb, fitc)

fitd <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + (1|year) + NAME_2 + (1|trial_id) + source, data = Tef_filtered)
anova(fitc, fitd)
...
saveRDS(yield_diff_mean, "~/shared-data/Data/Tef/fieldData/tef_modelReady.rds")
