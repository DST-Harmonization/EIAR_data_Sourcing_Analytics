...
#################################################################################################################
inputDataAOI <- readRDS("~/shared-data/Data/Tef/geoSpatial/tef_AOI.RDS")
pathOut = "~/shared-data/Data/Tef/geoSpatial/geo_4cropModel/"
...
for( zones in unique(inputDataAOI$Zone)){
  ...
  Eth_Tef_AOI_profileS1 <- extract_geoSpatialPointData(country = country, inputData=zone_inputData,
...
#################################################################################################################
pathOut = "~/shared-data/Data/Tef/geoSpatial/geo_4ML_AOI/"
inputDataAOI <- readRDS("~/shared-data/Data/Tef/geoSpatial/tef_AOI.RDS")
...
dat.files  <- list.files(path="~/shared-data/Data/Tef/geoSpatial/geo_4ML_AOI",
...
saveRDS(AOI_summary_weather2, "~/shared-data/Data/Tef/geoSpatial/geo_4ML_AOI/weatherSummaries_Season_1_AOI.RDS")
...
PlD_Tef <- readRDS("~/shared-data/Data/Tef/geoSpatial/Tef_pLHv_byZone.RDS")
head(PlD_Tef)
PlD_Tef$NAME_1 <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Tef$NAME_1)
PlD_Tef$Zone <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Tef$Zone)

inputDataTrial <- merge(inputDataTrial, PlD_Tef, by=c("Zone", "NAME_1"), all.x=TRUE)
## if there are trials outside the Tef belt and do not have planting and harvest dates,
...
PlD_Tef[PlD_Tef$NAME_1 %in% unique(inputDataTrial[is.na(inputDataTrial$planting_month_date),]$NAME_1), ]


unique(inputDataTrial[inputDataTrial$NAME_1 == "SNNP", ]$planting_month_date)
unique(inputDataTrial[inputDataTrial$NAME_1 == "SNNP", ]$harvest_month_date)
inputDataTrial1 <- inputDataTrial %>% 
  dplyr::filter(NAME_1 == "SNNP" & is.na(planting_month_date)) %>% 
  mutate(planting_month_date = "06-21",
         plMonth = "06",
         plDate = 21,
         HvWeeks= 12,
         pld= "2024-06-21",
         ...
         hvMonth=11,
         hvDate="28",
         harvest_month_date="11-28")


unique(inputDataTrial[inputDataTrial$NAME_1 == "Oromia", ]$planting_month_date)
unique(inputDataTrial[inputDataTrial$NAME_1 == "Oromia", ]$harvest_month_date)
inputDataTrial2 <- inputDataTrial %>% 
  dplyr::filter(NAME_1 == "Oromia" & is.na(planting_month_date)) %>% 
  mutate(planting_month_date = "06-21",
         plMonth = "06",
         plDate = 21,
         HvWeeks= 12,
         ...
         hvMonth=10,
         hvDate="19",
         harvest_month_date="10-19")

inputDataTrial3 <- inputDataTrial %>% 
  dplyr::filter(!is.na(planting_month_date)) 

inputDataTrial <- rbind(inputDataTrial1, inputDataTrial2, inputDataTrial3)
...
head(inputDataTrial)
inputDataTrial$ID <- inputDataTrial$TLID ## this is very important to link different data frames later, 

## get data for trial crop model
pathOut <- "~/shared-data/Data/Tef/geoSpatial/geo_4cropModel_trial/"
Tef_weatehr_soil_trial_profile <- extract_geoSpatialPointData(country = country, inputData=inputDataTrial,varsbasePath=varsbasePath,
                                                              AOI=FALSE, Planting_month_date=NULL, Harvest_month_date=NULL,
                                                              soilData = TRUE, weatherData = FALSE, soilProfile = TRUE, season = 1,
...
#################################################################################################################
pathOut <- "~/shared-data/Data/Tef/geoSpatial/geo_4ML_trial/"
