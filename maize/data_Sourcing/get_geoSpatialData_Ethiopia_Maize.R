...
#################################################################################################################
inputDataAOI <- readRDS("~/shared-data/Data/Maize/geoSpatial/Maize_AOI.RDS")
pathOut = "~/shared-data/Data/Maize/geoSpatial/geo_4cropModel/"
...
for( zones in unique(inputDataAOI$Zone)){
  ...
  Eth_Maize_AOI_profileS1 <- extract_geoSpatialPointData(country = country, inputData=zone_inputData,
...
#################################################################################################################
pathOut = "~/shared-data/Data/Maize/geoSpatial/geo_4ML_AOI/"
inputDataAOI <- readRDS("~/shared-data/Data/Maize/geoSpatial/Maize_AOI.RDS")
...
dat.files  <- list.files(path="~/shared-data/Data/Maize/geoSpatial/geo_4ML_AOI",
...
saveRDS(AOI_summary_weather2, "~/shared-data/Data/Maize/geoSpatial/geo_4ML_AOI/weatherSummaries_Season_1_AOI.RDS")
...
PlD_Maize <- readRDS("~/shared-data/Data/Maize/geoSpatial/Maize_pLHv_byZone.RDS")
head(PlD_Maize)
PlD_Maize$NAME_1 <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Maize$NAME_1)
PlD_Maize$Zone <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Maize$Zone)

inputDataTrial <- merge(inputDataTrial, PlD_Maize, by=c("Zone", "NAME_1"), all.x=TRUE)
## if there are trials outside the Maize belt and do not have planting and harvest dates,
...
inputDataTrial %>% dplyr::filter(NAME_1 == "SNNP" & is.na(planting_month_date))
inputDataTrial[inputDataTrial$NAME_1 == "SNNP", ]$planting_month_date <- "01-11"
inputDataTrial[inputDataTrial$NAME_1 == "SNNP", ]$harvest_month_date <- "05-10"

inputDataTrial %>% dplyr::filter(NAME_1 == "Oromia" & is.na(planting_month_date))
inputDataTrial[inputDataTrial$NAME_1 == "Oromia" & is.na(inputDataTrial$planting_month_date), ]$planting_month_date <- "02-21"
inputDataTrial[inputDataTrial$NAME_1 == "Oromia" & is.na(inputDataTrial$harvest_month_date), ]$harvest_month_date <- "07-20"
...
pathOut <- "~/shared-data/Data/Maize/geoSpatial/geo_4cropModel_trial/"
Maize_weather_soil_trial_profile <- extract_geoSpatialPointData(country = country, inputData=inputDataTrial,varsbasePath=varsbasePath,
...
#################################################################################################################
pathOut <- "~/shared-data/Data/Maize/geoSpatial/geo_4ML_trial/"
