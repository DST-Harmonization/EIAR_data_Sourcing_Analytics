...
```{r}
dim(Maize_joined)
sg_2000 <- Maize_joined |> dplyr::filter(source == "SG 2000")
dim(sg_2000)
yld_gr_7k <- Maize_joined |> dplyr::filter(grain_yield_kgpha > 12000)
Maize_filtered <- Maize_joined |> dplyr::filter(source != "SG 2000" & grain_yield_kgpha <= 12000)
dim(yld_gr_7k)
dim(Maize_filtered)
```
...
```{r warning=FALSE}
few_obs_data <- Maize_filtered |> dplyr::filter(trial_id %in% few_observs$trial_id)
...
Maize_filtered <- Maize_filtered |> dplyr::filter(!(trial_id %in% flagged_data$trial_id))
dim(Maize_filtered)
```
