...
```{r}
sorghum <- read.csv("~/shared-data/Data/Sorghum/fieldData/sorghum_dst_data_v1.csv", header = T, sep = ",", fileEncoding="latin1")
sorghum <- sorghum %>% 
  dplyr::rename(unique_id=Id, long = Long, lat = Lat,year=Year,source=Data.source,n_kgpha=N_amount, p_kgpha=P_amount, k_kgpha=K_amount ,treatment=Treat_Label.1, grain_yield_kgpha=Grain_yield )
names(sorghum)
head(sorghum)
# length(unique(sorghum$unique_id))
```
...
```{r}
hed_sorghum <- head(sorghum,3)
rmarkdown::paged_table(hed_sorghum, options=NULL) 
```

Let's check the number of NA values in each column
```{r}
sorghum_sumr <- sorghum |>
  summarise_all(~sum(is.na(.)))
rmarkdown::paged_table(sorghum_sumr, options=NULL) 
```
...
sorghum_pts <- terra::vect(sorghum, geom = c('long', 'lat'), crs = "epsg:4326") |> st_as_sf()
dim(sorghum_pts)
sorghum_joined <- st_join(sorghum_pts, eth_dist) 
sorghum_joined <- st_coordinates(sorghum_joined) |> cbind(as.data.frame(sorghum_joined))  |> dplyr::select(unique_id, X, Y, year, source, treatment, n_kgpha, p_kgpha, k_kgpha, grain_yield_kgpha, pl_date_ec, hv_date_ec, NAME_1, NAME_2, NAME_3)
head(sorghum_joined, 3)
```
...
```{r}
sorghum_joined <- sorghum_joined |> dplyr::mutate(long2 = round(X, digits = 2), lat2 = round(Y , digits = 2), trial_id = paste(long2, lat2, year, source, sep = "_"),trial_id2 = paste(X, Y, year, source, sep = "_"))  
...
tids_moreids <- sorghum_joined %>% 
  dplyr::group_by(trial_id, unique_id) %>%  
  dplyr::summarise(n = n()) 
tids_moreids[tids_moreids$n > 1, ]

sorghum_joined1 <- sorghum_joined[sorghum_joined$trial_id %in% tids_moreids$trial_id, ] 
dim(sorghum_joined)
sorghum_joined <- sorghum_joined %>% dplyr::select(-c(unique_id)) %>% unique()
dim(sorghum_joined)
length(unique(sorghum_joined$trial_id))
length(unique(sorghum_joined$trial_id2))
```

```{r}
sorghum_joined <- sorghum_joined |> dplyr::mutate(NAME_1 = replace(NAME_1, NAME_1 == "Southern Nations, Nationalities", "SNNP"))|> dplyr::mutate(n_rate2 = round_any(n_kgpha, 5), p_rate2 = round_any(p_kgpha, 2), k_rate2 = round_any(k_kgpha, 10)) |> dplyr::mutate(treatment_id = 
...
```{r}
unique(sorghum_joined[sorghum_joined$grain_yield_kgpha < 1000, ]$treatment_id)

dim(sorghum_joined %>% dplyr::filter(grain_yield_kgpha < 1000, ))
sorghum_joined <- sorghum_joined %>% dplyr::filter(grain_yield_kgpha > 1000, )
...
Check the observations that lies inside the sorghum crop mask area
...
```{r}
sorghum_joined2 <- sorghum_joined |> terra::vect(geom = c('X', 'Y'), crs = "epsg:4326") |> st_as_sf() 
ggplot() +
  geom_sf(data = eth,
          fill = NA,
          color = "black") +
  geom_sf(data = sorghum_joined2,
...
```{r}
flag1 <- sorghum_joined |> dplyr::filter(is.na(NAME_1) | is.na(NAME_2) | is.na(NAME_3))
...
```{r}
sorghum_filtered <- sorghum_joined |> dplyr::filter(!(trial_id %in% flag1$trial_id))
dim(sorghum_filtered)
```

## 7. Number of observations by region and global yield data distibution 
No of observations per region 
```{r}
sorghum_filtered |> dplyr::group_by(NAME_1) |> dplyr::summarize(n = n()) |>
  dplyr::arrange(desc(n))
```

```{r}
sorghum_filtered |> ggplot(aes(x = source, y = grain_yield_kgpha, fill = source))+
  geom_boxplot()+
  theme_bw()+
...
```{r}
sg_2000 <- sorghum_filtered |> dplyr::filter(source == "SG 2000")
dim(sg_2000)
yld_gr_7k <- sorghum_filtered |> dplyr::filter(grain_yield_kgpha > 7500)
sorghum_filtered <- sorghum_filtered |> dplyr::filter(source != "SG 2000" & grain_yield_kgpha <= 7500)
dim(yld_gr_7k)
dim(sorghum_filtered)
```
...
```{r}
for(i in c(9, 19:21)){
  message(colnames(sorghum_filtered)[i])
  print(summary(sorghum_filtered[,i]))
}
```
Distribution of treatments and yield
```{r}
par(mfrow=c(2,2))
hist(sorghum_filtered$grain_yield_kgpha, xlab = "yield", main = "")
hist(sorghum_filtered$n_rate2, xlab = "N rate", main = "")
hist(sorghum_filtered$p_rate2, xlab = "P rate", main = "")
hist(sorghum_filtered$k_rate2, xlab = "K rate", main = "")
```
## 10. Number of observations by year
Summarize data collected in different year
```{r  warning=FALSE, fig.width=10}
sorghum_filtered |> ggplot(aes(year))+
...
```{r}
obs_src <- sorghum_filtered |> dplyr::group_by(source) |> dplyr::summarise(n = n()) |> 
  dplyr::arrange(desc(n))
...
```{r}
obs_trt <- sorghum_filtered |> dplyr::group_by(treatment_id) |> dplyr::summarise(n = n()) |> dplyr::arrange(desc(n))
rmarkdown::paged_table(obs_trt, options=NULL) 
```

## 13. Number of observations by data treatment and source
```{r}
obs_trt_src <- sorghum_filtered |> dplyr::group_by(source, treatment_id) |> dplyr::summarize(n = n()) |>
  dplyr::arrange(desc(n))
...
```{r}
obs_src_trid <- sorghum_filtered |> dplyr::group_by(source, trial_id) |> dplyr::summarize(n = n()) |> dplyr::arrange(desc(n))
...
```{r fig.height= 7}
few_observs <- sorghum_filtered |> dplyr::group_by(trial_id) |> dplyr::summarise(n = n()) |> dplyr::filter(n == 1)
sorghum_few_observs <- sorghum_filtered |> dplyr::filter((trial_id %in% few_observs$trial_id))
sorghum_few_observs |> ggplot(aes(x = source, y = grain_yield_kgpha, fill = source))+
  geom_point()+
  facet_wrap(~year)+
  theme_bw()+
...
```{r warning=FALSE}
few_obs_data <- sorghum_filtered |> dplyr::filter(trial_id %in% few_observs$trial_id)
...
```{r}
sorghum_filtered <- sorghum_filtered |> dplyr::filter(!(trial_id %in% flagged_data$trial_id))
dim(sorghum_filtered)
```
...
```{r}
sorghum_filtered$index <- c(1:nrow(sorghum_filtered))
orgIndex <- grep("Org", sorghum_filtered$treatment)
orgdata <- droplevels(sorghum_filtered[sorghum_filtered$index %in% orgIndex, ])
sorghum_filtered <- droplevels(sorghum_filtered[!sorghum_filtered$index %in% orgIndex, ])
dim(sorghum_filtered)
```
...
```{r}
sorghum_filtered$index <- paste(sorghum_filtered$trial_id, sorghum_filtered$treatment_id, sep="_")
dd <- NULL
for (tids in unique(sorghum_filtered$trial_id)){
  tdata <- sorghum_filtered[sorghum_filtered$trial_id == tids, ]
...
Let's filter out the above observations from the sorghum filtered data
```{r}
dim(sorghum_filtered)
length(unique(sorghum_filtered$trial_id))
sorghum_filtered <- sorghum_filtered %>% dplyr::filter(!index %in% dd$index) %>% unique()
dim(sorghum_filtered)
length(unique(sorghum_filtered$trial_id))
```
...
```{r}
obs_src_reg <- sorghum_filtered |> dplyr::group_by(source, NAME_1) |> dplyr::summarize(n = n()) |>
  dplyr::arrange(desc(n))
...
```{r}
obs_reg_trt <- sorghum_filtered |> dplyr::group_by(treatment_id, NAME_1) |> dplyr::summarize(n = n()) |>
...
```{r}
avg_yld_zone <- sorghum_filtered |> dplyr::group_by(NAME_1, NAME_2) |> dplyr::summarize(mean_yield = mean(grain_yield_kgpha)) |> dplyr::arrange(desc(mean_yield))
...
```{r}
avg_yld_dist <- sorghum_filtered |> dplyr::group_by(location) |> dplyr::summarize(mean_yield = mean(grain_yield_kgpha)) |> dplyr::arrange(desc(mean_yield))
rmarkdown::paged_table(avg_yld_dist, options=NULL) 
```

Few Observations by year
```{r}
obs_few_year <- sorghum_filtered |> dplyr::group_by(year) |> dplyr::summarise(n = n()) |> dplyr::arrange(n)
rmarkdown::paged_table(obs_few_year, options=NULL) 
```

Observations by year
```{r}
few_year <- sorghum_filtered |> dplyr::filter(year %in% c(1992, 1990, 1991, 2008, 2009, 2010)) |> 
...
```{r}
sorghum_filtered |> dplyr::filter(year %in% c(1992, 1990, 1991, 2008, 2009, 2010)) |>
...
sorghum_filtered2 <- sorghum_filtered |> 
...
```{r}
sorghum_filtered <- sorghum_filtered |> dplyr::filter(grain_yield_kgpha <= 15000) 
sorghum_filtered |> 
...
<!-- ```{r} -->
<!-- unq_tr_id <- sorghum_filtered |> dplyr::filter(grain_yield_kgpha >= 7500 & grain_yield_kgpha <= 15000) -->
<!-- unq_tr_id <- unique(unq_tr_id$trial_id2) -->
<!-- unq_tr_id2 <- sorghum_filtered |> dplyr::filter(grain_yield_kgpha <= 15000 & source %in% c("ICRISAT_ETH_Africa Rising", "ATA_Balanced_Fertilizer_Trial", "SG 2000") & year %in% c(2014, 2015, 2016) & trial_id %in% unq_tr_id) -->
...
<!-- ```{r} -->
<!-- col_high_yld <- sorghum_filtered |> dplyr::mutate(cy = ifelse(trial_id %in% unq_tr_id2$trial_id, 'green', 'red')) -->
<!-- col_high_yld |>  -->
...
```{r}
sorghum_filtered |> dplyr::filter(treatment_id == "0_0_0") |>
...
sorghum_filtered |> dplyr::filter(treatment_id == "0_0_0" & year %in% c(2020,2021) & NAME_1 %in% c('Amhara', 'Oromia', 'SNNP')) |>
...
```{r, fig.height=10, fig.width=13}
ctrl_high_yld <- sorghum_filtered |> dplyr::filter(treatment_id == "0_0_0"  & grain_yield_kgpha >= 3500 & grain_yield_kgpha <= 15000) 
sorghum_filtered |> dplyr::filter(trial_id %in% unique(ctrl_high_yld$trial_id)) |>
ggplot(aes(y = grain_yield_kgpha, x = NAME_2, fill = as.factor(year)))+
  geom_boxplot()+
...
unique_tr_id <- unique(sorghum_filtered$trial_id)
yield_diff_mean <- data.frame()
for(i in 1:length(unique_tr_id)){
  unique_tr_yld <- sorghum_filtered |> dplyr::filter(trial_id ==  unique_tr_id[i])
...
saveRDS(yield_diff_mean, "~/shared-data/Data/sorghum/fieldData/yield_diff1.rds")
```
...
sorghum_filtered$year <- as.factor(sorghum_filtered$year)
sorghum_filtered$source <- as.factor(sorghum_filtered$source)
sorghum_filtered$location <- as.factor(sorghum_filtered$location)
sorghum_filtered$trial_id <- as.factor(sorghum_filtered$trial_id)
sorghum_filtered$NAME_2 <- as.factor(sorghum_filtered$NAME_2)

sorghum_filtered$N100 <- sorghum_filtered$n_rate2/100
sorghum_filtered$P100 <- sorghum_filtered$p_rate2/100
sorghum_filtered$K100 <- sorghum_filtered$k_rate2/100

fita <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + (1|trial_id), data = sorghum_filtered)

fitb <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + (1|trial_id) + source, data = sorghum_filtered)
anova(fita, fitb)

fitc <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + year + NAME_2 + (1|trial_id) + source, data = sorghum_filtered)
anova(fitb, fitc)

fitd <- lmer(sqrt(grain_yield_kgpha) ~ n_rate2 + p_rate2 + k_rate2 + (1|year) + NAME_2 + (1|trial_id) + source, data = sorghum_filtered)
anova(fitc, fitd)
...
sorghum_filtered$blup <- predict(fitg, sorghum_filtered)**2
```
...
saveRDS(yield_diff_mean, "~/shared-data/Data/sorghum/fieldData/yield_diff_blup1.rds")
