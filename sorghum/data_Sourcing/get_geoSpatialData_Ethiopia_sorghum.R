...
#################################################################################################################
inputDataAOI <- readRDS("~/shared-data/Data/Sorghum/geoSpatial/sorghum_AOI.RDS")
pathOut = "~/shared-data/Data/Sorghum/geoSpatial/geo_4cropModel/"
...
for( zones in unique(inputDataAOI$Zone)){
  ...
  Eth_Sorghum_AOI_profileS1 <- extract_geoSpatialPointData(country = country, inputData=zone_inputData,
...
#################################################################################################################
## get the geo spatial data for AOI for ML prediction
#################################################################################################################
pathOut = "~/shared-data/Data/Sorghum/geoSpatial/geo_4ML_AOI/"
inputDataAOI <- readRDS("~/shared-data/Data/Sorghum/geoSpatial/sorghum_AOI.RDS")

AOI_ML_soil <- extract_geoSpatialPointData(country = country, inputData=inputDataAOI, varsbasePath=varsbasePath, pathOut= pathOut,
...
for(zones in unique(inputDataAOI$Zone)){
  ...
  ## Data for crop models at AOI : weather + the 6 profiles of soil grids soil data
  AOI_ML_weather <- get_WeatherSummarydata(country = country, inputData = zone_inputData, AOI = TRUE, varsbasePath=varsbasePath,zone = zones,
...
dat.files  <- list.files(path="~/shared-data/Data/Sorghum/geoSpatial/geo_4ML_AOI", recursive=TRUE, full.names=TRUE)
...
saveRDS(AOI_summary_weather2, "~/shared-data/Data/Sorghum/geoSpatial/geo_4ML_AOI/weatherSummaries_Season_1_AOI.RDS")
...
#################################################################################################################
PlD_Sorghum <- readRDS("~/shared-data/Data/Sorghum/geoSpatial/Sorghum_pLHv_byZone.RDS")
head(PlD_Sorghum)
PlD_Sorghum$NAME_1 <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Sorghum$NAME_1)
PlD_Sorghum$Zone <- gsub("Southern Nations, Nationalities", "SNNP", PlD_Sorghum$Zone)
...
inputDataTrial$NAME_1 <- gsub("Southern Nations, Nationalities", "SNNP", inputDataTrial$NAME_1)

inputDataTrial <- inputDataTrial %>% 
  dplyr::select(trail_id, Year, NAME_1, NAME_2, Long, Lat,Planting.date, Harvesting.date) %>% 
  dplyr::rename(TLID = trail_id, lon=Long, lat=Lat) %>% 
  dplyr::mutate(Zone = paste(NAME_1, NAME_2, sep="_")) %>% 
  unique()
head(inputDataTrial)

### inputDataTrial with planting and harvest dates
inputDataTrial1 <- inputDataTrial %>% dplyr::filter(Planting.date != "", )
inputDataTrial1$plantingDate <-  dmy(inputDataTrial1$Planting.date)
inputDataTrial1$harvestDate <- dmy(inputDataTrial1$Harvesting.date)
inputDataTrial1[is.na(inputDataTrial1$harvestDate), ]
inputDataTrial1[inputDataTrial1$Zone == "Amhara_North Shewa", ]
inputDataTrial1[is.na(inputDataTrial1$harvestDate), ]$harvestDate <- "2017-12-27"


inputDataTrial1 <- inputDataTrial1 %>% 
  dplyr::select("TLID", "Year", "NAME_1", "NAME_2","lon", "lat", "plantingDate","harvestDate") %>% 
  unique()



#### inputDataTrial without planting and harvest dates
inputDataTrial2 <- inputDataTrial %>% dplyr::filter(Planting.date =="", )
inputDataTrial2 <- merge(inputDataTrial2, PlD_Sorghum, by=c("Zone", "NAME_1"), all.x=TRUE)
inputDataTrial2 <- inputDataTrial2 %>% 
  dplyr::mutate(plantingDate =  paste(Year, planting_month_date, sep="-"),
                harvestDate = paste(Year, harvest_month_date, sep="-")) %>% 
  dplyr::select("TLID", "Year", "NAME_1", "NAME_2","lon", "lat", "plantingDate","harvestDate") %>% 
  unique()


inputDataTrial <- rbind(inputDataTrial1, inputDataTrial2)
...
pathOut <- "~/shared-data/Data/Sorghum/geoSpatial/geo_4cropModel_trial/"
Sorghum_weather_soil_trial_profile <- extract_geoSpatialPointData(country = country, inputData=inputDataTrial,varsbasePath=varsbasePath,
...
#################################################################################################################
pathOut <- "~/shared-data/Data/Sorghum/geoSpatial/geo_4ML_trial/"
